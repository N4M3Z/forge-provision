...
Best-practice provisioning for developer machines — macOS, Linux, and Windows — plus the agentic scaffold (skills, ADRs, rules, TLDRs, journals) that lets an agent (Codex, Codex, Gemini) read, compose, and run the scripts. See [ARCH-0001](docs/decisions/ARCH-0001%20Module%20scope%20cross-platform%20provisioning.md) for the module scope.
...
│   ├── install/              # install software — Codex, brew, chezmoi, pass-cli, …
...
2. Name the file after the **target** (the noun): `install/brew.sh`, `clone/references.sh`, `migrate/Codex-history.sh`. No numbered prefixes; flat-in-verb mirrors check-mac's flat-in-topic.
